...
let jack = make_friend "Jack" 5 
let nicola = make_friend "Nicola" 5
let gandhi = make_friend "Gandhi" 5
let maximillian = make_friend "Maximillian the III" 5
let lirinda = make_friend "Lirinda" 5 
let sam = make_friend "Sam" 5
let brad = make_friend "Brad" 5
let sadman = make_friend "Sadman" 5
let charlie = make_friend "Charlie" 5
...
(** [make_scenario] takes in types name, prompt, and choices
    and produces a new scenario type. *)
let make_scenario name prompt choices =
  {
    name = name;
    prompt = prompt;
    choices = choices;
...
let stir_fry = make_scenario "stir fry" 
    ...
    ["Do the right thing"; "Screw Okies"]  
...
let wsh = make_scenario "WSH" 
    ...
    ["Visit Denice Cassaro"; "Okenshield's"; "Watch a movie" ] 
...
let canada_goose = make_scenario "Canada Goose" 
    ...
    ["Canada Goose Jacket"; "Jacket from TJ Maxx"]
...
let important_class = make_scenario "important class"
    ...
    ~their spot."
    ["Take the offer"; "Eh, get some electives done"] 
...
let rec closeness_helper decision list student= 
  ...
  | [] -> ("false", 0)
  | h :: t -> 
    if fst h = decision && Student.see_if_you_have_friend (fst (snd h)) student
    then snd h 
    else closeness_helper decision t student
...
let main_closeness_function decision student = 
  ...
  if fst tuple = "false" then student else 
    ...
    in 
    let new_friend = Friend.update_friend friend (snd tuple) in  
